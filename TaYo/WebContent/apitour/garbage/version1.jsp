...
// 		도시버튼 클릭 이벤트 추가
		$(document).on("click", ".filter_body.si button",function() {
			console.log($(this));
			console.log('도시코드'+$(this).children('span').attr("class"));
			...
			$(filterItem).on("click", function() { //필터에 넣은 아이템 클릭시 이벤트
				parentTr.css("display", "table-row");
				$(this).remove();
			});
			
			//필터에 넣을 아이템추가
			$('body > div.container.h-100.pt-3 > table > tbody > tr:nth-child(1) > td.filter_body').
			append(filterItem);			
		});
